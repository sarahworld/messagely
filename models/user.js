...
class User {
  ...
  static async register({username, password, first_name, last_name, phone}) { 
    ...
    const results = db.query(`INSERT INTO user (
      username, 
      password, 
      first_name, 
      last_name,
  ...
  static async updateLoginTimestamp(username) { 
    const results = await db.query(`UPDATE users
                            ...
                            [username])
    if(!results){
      throw new ExpressError(`No user with such ${username}`, 404)
    }
  }
  ...
  static async get(username) {
    const results = await db.query(`SELECT username,
                           ...
                           [username])
    if(!results){
      throw new ExpressError(`No user with this ${username}`, 404)
    }
    return results.rows[0]
  ...
  static async messagesFrom(username) {
    const results = await db.query(`SELECT 
                              ...
                              u.phone, m.body, m.sent_at, m.read_at
                              FROM messages AS m
